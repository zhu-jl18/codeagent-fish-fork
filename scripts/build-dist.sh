...
(
  ...
  CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o "$OUT_DIR/codeagent-wrapper-darwin-arm64"
)

chmod +x "$OUT_DIR/codeagent-wrapper-linux-amd64" "$OUT_DIR/codeagent-wrapper-darwin-arm64" || true
